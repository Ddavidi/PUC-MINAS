/*
   ==UserScript==
 @name         TP04Q02 - Árvore Binária de Árvore Binárias em Java
 @namespace    https://github.com/Ddavidi/PUC-MINAS
 @description  VERDE PUC MINAS - TP04Q02 - Árvore Binária de Árvore Binárias em Java
 @author       @ddavidi_
   ==/UserScript==
*/
...
public class ArvoreBinariaDeArvoreBinariaJava {
...
class  ArvoreBinaria{
    ...
    public void inserir(Show show) {
        raiz = inserirRecursivo(raiz, show);
    }

    private No inserirRecursivo(No no, Show show) {
        if (no == null) {
            return new No(show);
        }
        if (show.getTitle().compareTo(no.getShow().getTitle()) < 0) {
            no.setEsquerda(inserirRecursivo(no.getEsquerda(), show));
        } else if (show.getTitle().compareTo(no.getShow().getTitle()) > 0) {
            no.setDireita(inserirRecursivo(no.getDireita(), show));
        }
        return no;
    }
    ...
    private boolean pesquisarRecursivo(No no, String titulo) {
        ...
        if (titulo.equals(no.getShow().getTitle())) {
            return true;
        }
        if (titulo.compareTo(no.getShow().getTitle()) < 0) {
            System.out.print("esq ");
            return pesquisarRecursivo(no.getEsquerda(), titulo);
        } else {
            System.out.print("dir ");
            return pesquisarRecursivo(no.getDireita(), titulo);
        }
    }
}

class NoArvoreDeArvore {
    int chave; // releaseYear % 15
    ArvoreBinaria arvoreSecundaria;
    NoArvoreDeArvore esquerda, direita;

    public NoArvoreDeArvore(int chave) {
        this.chave = chave;
...
class ArvoreDeArvores {
    private NoArvoreDeArvore raiz;

    public ArvoreDeArvores() {
        int[] ordem = {7, 3, 11, 1, 5, 9, 13, 0, 2, 4, 6, 8, 10, 12, 14};
        for (int chave : ordem) {
            raiz = inserirNoFixo(raiz, chave);
        }
    }

    private NoArvoreDeArvore inserirNoFixo(NoArvoreDeArvore no, int chave) {
        if (no == null) return new NoArvoreDeArvore(chave);
        if (chave < no.chave) {
            no.esquerda = inserirNoFixo(no.esquerda, chave);
        } else if (chave > no.chave) {
            no.direita = inserirNoFixo(no.direita, chave);
        }
        return no;
    }
    ...
    public void inserir(Show show) {
        ...
        raiz = inserirRec(raiz, chave, show);
    }

    private NoArvoreDeArvore inserirRec(NoArvoreDeArvore no, int chave, Show show) {
        if (no == null) {
            NoArvoreDeArvore novo = new NoArvoreDeArvore(chave);
            novo.arvoreSecundaria.inserir(show);
            return novo;
        }
        if (chave < no.chave) {
            no.esquerda = inserirRec(no.esquerda, chave, show);
        } else if (chave > no.chave) {
            no.direita = inserirRec(no.direita, chave, show);
        } else {
            // Se já existe, insere na árvore secundária
            no.arvoreSecundaria.inserir(show);
        }
        return no;
    }
...
private boolean pesquisarRec(NoArvoreDeArvore no, String titulo) {
    if (no == null) return false;

    boolean encontrou = no.arvoreSecundaria.pesquisar(titulo);
    if (encontrou) return true;

        System.out.print(" ESQ ");
    if (pesquisarRec(no.esquerda, titulo)) {
        return true;
    }

    System.out.print(" DIR ");
    if (pesquisarRec(no.direita, titulo)) {
        return true;
    }
