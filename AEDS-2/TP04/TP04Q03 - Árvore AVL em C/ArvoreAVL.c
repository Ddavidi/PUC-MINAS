/*
   ==UserScript==
 @name         TP04Q03 - Árvore AVL em C
 @namespace    https://github.com/Ddavidi/PUC-MINAS
 @description  VERDE PUC MINAS - TP04Q03 - Árvore AVL em C
 @author       @ddavidi_
   ==/UserScript==
*/
...
void atualizarAltura(No* no) {
    if (no != NULL) {
        ...
        no->nivel = 1 + (altEsq > altDir ? altEsq : altDir);
    }
}
...
No* inserirRec(No* no, Show* show) {
    if (no == NULL) {
        return novoNo(show);
    }
    if (strcmp(show->title, no->data->title) < 0) {
        no->esq = inserirRec(no->esq, show);
    } else if (strcmp(show->title, no->data->title) > 0) {
        no->dir = inserirRec(no->dir, show);
    } else {
        return no;
    }
    atualizarAltura(no);
    int fb = fatorBalanceamento(no);
    if (fb > 1 && strcmp(show->title, no->esq->data->title) < 0) {
        return rotacaoDireita(no);
    }
    if (fb < -1 && strcmp(show->title, no->dir->data->title) > 0) {
...
int pesquisarRec(char* titulo, No* no) {
    int resp = 0;
    if (no == NULL) {
        resp = 0;
    } else if (strcmp(titulo, no->data->title) < 0) {
        printf("esq ");
        resp = pesquisarRec(titulo, no->esq);
    } else if (strcmp(titulo, no->data->title) > 0) {
        printf("dir ");
        resp = pesquisarRec(titulo, no->dir);
    } else {
        resp = 1;
    }
    return resp;
...
int pesquisar(No* raiz,char* titulo) {
    printf("raiz ");
	return pesquisarRec(titulo, raiz);
}
...
int main() {
    ...
    ArvoreAVL* arvore = novaArvore();
    char entrada[100];
    scanf(" %s", entrada);
    while (strcmp(entrada, "FIM") != 0) {
        int index = atoi(entrada + 1) - 1;
        if (index >= 0 && index < total) {
            arvore->raiz = inserirRec(arvore->raiz, &shows[index]);
        } else {
    ...
    scanf(" %[^\n]", entrada);
    while (strcmp(entrada, "FIM") != 0) {
        if (pesquisar(arvore->raiz, entrada) == 1) {
            printf("SIM\n");
        } else {
            printf("NAO\n");
        }
