/*
   ==UserScript==
 @name         TP02Q11 - Counting Sort em Java
 @namespace    https://github.com/Ddavidi/PUC-MINAS
 @description  VERDE PUC MINAS - TP02Q11 - Counting Sort em Java
 @author       @ddavidi_
   ==/UserScript==
*/
...
public class Principal {
    public static void main(String[] args) {
        ...
        Show.countingSort(arrayShows, i);
        for (int j = 0; j < i; j++) {
...
class Show {
    ...
    public static void countingSort(Show[] shows, int maxElements) {
        if (shows == null || shows.length <= 1 || maxElements <= 0) return;

        int numToSort = Math.min(maxElements, shows.length);

        int minYear = Integer.MAX_VALUE;
        int maxYear = Integer.MIN_VALUE;

        for (int i = 0; i < numToSort; i++) {
            if (shows[i].release_year < minYear) minYear = shows[i].release_year;
            if (shows[i].release_year > maxYear) maxYear = shows[i].release_year;
        }

        int range = maxYear - minYear + 1;
        Show[][] buckets = new Show[range][];
        
        for (int i = 0; i < range; i++) {
            buckets[i] = new Show[shows.length];
        }

        for (int i = 0; i < numToSort; i++) {
            int index = shows[i].release_year - minYear;
            int j = 0;
            while (buckets[index][j] != null) {
                j++;
            }
            buckets[index][j] = shows[i];
        }
        ...
        for (int i = 0; i < numToSort - 1; i++) {
            for (int j = i + 1; j < numToSort; j++) {
                if (shows[i].release_year == shows[j].release_year) {
                    // Comparar por título (ordem lexicográfica)
                    if (shows[i].title.toLowerCase().compareTo(shows[j].title.toLowerCase()) > 0) {
                        // Trocar os shows
                        Show temp = shows[i];
                        shows[i] = shows[j];
                        shows[j] = temp;
