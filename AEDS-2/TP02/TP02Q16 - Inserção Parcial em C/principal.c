/*
   ==UserScript==
 @name         TP02Q16 - Inserção Parcial em C
 @namespace    https://github.com/Ddavidi/PUC-MINAS
 @description  VERDE PUC MINAS - TP02Q16 - Inserção Parcial em C
 @author       @ddavidi_
   ==/UserScript==
*/
...
void insertionSortTypeTitle(Show* array, int size) {
    for (int i = 1; i < size; i++) {
        Show key = array[i];

        char lowerKeyType[300];
        char lowerKeyTitle[300];
        toLowerString(key.type, lowerKeyType);
        toLowerString(key.title, lowerKeyTitle);

        int j = i - 1;
        while (j >= 0) {
            char lowerJType[300];
            char lowerJTitle[300];
            toLowerString(array[j].type, lowerJType);
            toLowerString(array[j].title, lowerJTitle);

            int cmp = strcmp(lowerJType, lowerKeyType);
            if (cmp > 0 || (cmp == 0 && strcmp(lowerJTitle, lowerKeyTitle) > 0)) {
                array[j + 1] = array[j];
                j--;
            } else {
                break;
