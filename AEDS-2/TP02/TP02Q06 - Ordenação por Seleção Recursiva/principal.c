/*
   ==UserScript==
 @name         TP02Q06 - Ordenação por Seleção Recursiva
 @namespace    https://github.com/Ddavidi/PUC-MINAS
 @description  VERDE PUC MINAS - TP02Q06 - Ordenação por Seleção Recursiva
 @author       @ddavidi_
   ==/UserScript==
*/
...
// Ordenação por seleção recursiva com comparação case-insensitive
void selectionSortRecursivo(Show* array, int inicio, int tamanho) {
    if (inicio >= tamanho - 1) return; // Condição de parada

    int menor = inicio;
    char title1[256], title2[256];

    for (int i = inicio + 1; i < tamanho; i++) {
        toLowerString(array[i].title, title1);
        toLowerString(array[menor].title, title2);
    ...
    if (menor != inicio) {
        swapShow(&array[inicio], &array[menor]);
    }

    selectionSortRecursivo(array, inicio + 1, tamanho);
}
...
int main() {
    ...
    selectionSortRecursivo(arr, 0, i);
