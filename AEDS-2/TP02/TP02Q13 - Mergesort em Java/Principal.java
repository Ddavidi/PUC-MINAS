/*
   ==UserScript==
 @name         TP02Q13 - Mergesort em Java
 @namespace    https://github.com/Ddavidi/PUC-MINAS
 @description  VERDE PUC MINAS - TP02Q13 - Mergesort em Java
 @author       @ddavidi_
   ==/UserScript==
*/
...
public class Principal {
    public static void main(String[] args) {
        ...
        Show.mergesort(arrayShows, 0, i - 1);
        for (int j = 0; j < i; j++) {
...
class Show {
    ...
    // Função merge que combina duas metades ordenadas
    public static void merge(Show[] array, int left, int middle, int right) {
        int n1 = middle - left + 1;
        int n2 = right - middle;

        Show[] leftArray = new Show[n1];
        Show[] rightArray = new Show[n2];
        ...
        while (i < n1 && j < n2) {
            if (leftArray[i].getDuration().compareTo(rightArray[j].getDuration()) < 0) {
                array[k++] = leftArray[i++];
            } else if (leftArray[i].getDuration().compareTo(rightArray[j].getDuration()) > 0) {
                array[k++] = rightArray[j++];
            } else {
                // Se as durações forem iguais, compara o título
                if (leftArray[i].getTitle().toLowerCase().compareTo(rightArray[j].getTitle().toLowerCase()) <= 0) {
                    array[k++] = leftArray[i++];
                } else {
                    array[k++] = rightArray[j++];
                }
