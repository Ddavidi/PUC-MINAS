/*
   ==UserScript==
 @name         TP02Q17 - Heapsort Parcial em C
 @namespace    https://github.com/Ddavidi/PUC-MINAS
 @description  VERDE PUC MINAS - TP02Q17 - Heapsort Parcial em C
 @author       @ddavidi_
   ==/UserScript==
*/
...
void heapifyShowDirectorTitle(Show* array, int size, int i) {
    int largest = i;
    int left = 2 * i + 1;
    int right = 2 * i + 2;

    char lowerLargestDirector[300], lowerLeftDirector[300], lowerRightDirector[300];
    char lowerLargestTitle[300], lowerLeftTitle[300], lowerRightTitle[300];

    toLowerString(array[largest].director, lowerLargestDirector);
    toLowerString(array[largest].title, lowerLargestTitle);

    if (left < size) {
        toLowerString(array[left].director, lowerLeftDirector);
        toLowerString(array[left].title, lowerLeftTitle);
        int cmp = strcmp(lowerLeftDirector, lowerLargestDirector);
        if (cmp > 0 || (cmp == 0 && strcmp(lowerLeftTitle, lowerLargestTitle) > 0)) {
            largest = left;
            strcpy(lowerLargestDirector, lowerLeftDirector);
            strcpy(lowerLargestTitle, lowerLeftTitle);
        }
    }

    if (right < size) {
        toLowerString(array[right].director, lowerRightDirector);
        toLowerString(array[right].title, lowerRightTitle);
        int cmp = strcmp(lowerRightDirector, lowerLargestDirector);
        if (cmp > 0 || (cmp == 0 && strcmp(lowerRightTitle, lowerLargestTitle) > 0)) {
            largest = right;
        }
    }

    if (largest != i) {
        swapShow(&array[i], &array[largest]);
        heapifyShowDirectorTitle(array, size, largest);
    }
}

void heapSortDirectorTitle(Show* array, int size) {
    for (int i = size / 2 - 1; i >= 0; i--) {
        heapifyShowDirectorTitle(array, size, i);
    }

    for (int i = size - 1; i > 0; i--) {
        swapShow(&array[0], &array[i]);
        heapifyShowDirectorTitle(array, i, 0);
    }
}
