/*
   ==UserScript==
 @name         TP02Q12 - Bolha em C
 @namespace    https://github.com/Ddavidi/PUC-MINAS
 @description  VERDE PUC MINAS - TP02Q12 - Bolha em C
 @author       @ddavidi_
   ==/UserScript==
*/
...
int compararDatas(Show* a, Show* b) {
...
void bubbleSort(Show *array, int tamanho) {
    for (int i = 0; i < tamanho - 1; i++) {
        for (int j = 0; j < tamanho - 1 - i; j++) {
            int comparacao = compararDatas(&array[j], &array[j + 1]);
            char title1[256], title2[256];
            toLowerString(array[j].title, title1);
            toLowerString(array[j + 1].title, title2);
            if (comparacao > 0 || (comparacao == 0 && strcmp(title1, title2) > 0)) {
