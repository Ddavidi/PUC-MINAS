/*
   ==UserScript==
 @name         TP02Q08 - Shellsort em C
 @namespace    https://github.com/Ddavidi/PUC-MINAS
 @description  VERDE PUC MINAS - TP02Q08 - Shellsort em C
 @author       @ddavidi_
   ==/UserScript==
*/
...
void shellSort(Show* array, int tamanho) {
    ...
    while (h > 0) {
        ...
        for (int i = h; i < tamanho; i++) {
            ...
            char tempType[256], tempTitle[256];
            toLowerString(temp.type, tempType);
            toLowerString(temp.title, tempTitle);

            while (j >= h) {
                char currentType[256], currentTitle[256];
                toLowerString(array[j - h].type, currentType);
                toLowerString(array[j - h].title, currentTitle);

                int typeCmp = strcmp(tempType, currentType);
                int titleCmp = strcmp(tempTitle, currentTitle);

                if (typeCmp < 0 || (typeCmp == 0 && titleCmp < 0)) {
                    array[j] = array[j - h];
                    j -= h;
                } else {
                    break;
