/*
   ==UserScript==
 @name         TP02Q14 - Radixsort em C
 @namespace    https://github.com/Ddavidi/PUC-MINAS
 @description  VERDE PUC MINAS - TP02Q14 - Radixsort em C
 @author       @ddavidi_
   ==/UserScript==
*/
...
int getMaxReleaseYear(Show* array, int size) {
...
void countingSortByReleaseYear(Show* array, int size, int exp) {
    Show output[size];
    int count[10] = {0};
    for (int i = 0; i < size; i++) {
        int digit = getDigit(array[i].release_year, exp);
        count[digit]++;
    }
    for (int i = 1; i < 10; i++)
        count[i] += count[i - 1];
    ...
    while (start < size) {
        int end = start + 1;
        while (end < size && array[end].release_year == array[start].release_year)
            end++;
        for (int i = start; i < end - 1; i++) {
            for (int j = start; j < end - 1 - (i - start); j++) {
                char title1[300], title2[300];
                toLowerString(array[j].title, title1);
                toLowerString(array[j + 1].title, title2);
                if (strcmp(title1, title2) > 0) {
                    swapShow(&array[j], &array[j + 1]);
                }
            }
        }
        start = end;
...
void radixSortByReleaseYear(Show* array, int size) {
    int max = getMaxReleaseYear(array, size);

    for (int exp = 1; max / exp > 0; exp *= 10) {
        countingSortByReleaseYear(array, size, exp);
    }
}
